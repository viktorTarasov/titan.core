...
namespace Common {
  ...
  bool ContainedSubtypeConstraint::get_constraints_def(struct_constraints *cons_def)
  {
    ...
    ec_core = mputprintf(ec_core, "        ec->add_msg(\": Failed to validate SubType constraint defined at (%s:%i) :\");\n",
		    get_filename(), get_first_line());
  ...
  bool NamedConstraint::get_constraints_def(struct_constraints *cons_def)
  {
    ...
    if (presence_constraint == PC_NONE)   {
      ...
      if (presence_constraint == PC_PRESENT)   {
	    ...
	    cons_def->con_str = mputstr(cons_def->con_str, "  if (ec != NULL)\n");
	    cons_def->con_str = mputprintf(cons_def->con_str, "    ec->add_msg(\": Failed to validate constraint 'PRESENT' defined at (%s:%i) :\");\n", get_filename(), get_first_line());
	    ...
	    cons_def->con_str = mputstr(cons_def->con_str, "if(field_present)  {\n");
	    cons_def->con_str = mputstr(cons_def->con_str, "  if (ec != NULL)\n");
	    cons_def->con_str = mputprintf(cons_def->con_str, "    ec->add_msg(\": Failed to validate constraint 'ABSENT' defined at (%s:%i) :\");\n", get_filename(), get_first_line());
	    cons_def->con_str = mputstr(cons_def->con_str, "  return FALSE;\n");
	    cons_def->con_str = mputstr(cons_def->con_str, "}\n");
          }
	}
