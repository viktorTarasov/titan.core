...
void Type::generate_code_coding_handlers(output_struct* target)
{
  ...
  if (constr_defs != NULL)  {
    target->header.function_prototypes = mputprintf( target->header.function_prototypes,
      "extern bool %s_validate_constraints(const %s& value, TTCN_EncDec_ErrorContext *ec);\n",
      get_genname_own().c_str(), get_genname_value(my_scope).c_str());

    target->header.function_prototypes = mputprintf( target->header.function_prototypes,
      "extern bool %s_validate_value(const void *value, TTCN_EncDec_ErrorContext *ec);\n", get_genname_own().c_str());

    validate_constraints = mprintf(
      "bool %s_validate_constraints(const %s& value, TTCN_EncDec_ErrorContext *ec)\n"
      "{\n"
    ...
    validate_constraints = mputprintf(validate_constraints,
      "bool %s_validate_value(const void *value, TTCN_EncDec_ErrorContext *ec)\n"
      "{\n"
      "if(value == NULL)  return FALSE;\n"
      "const %s *typed_value = (%s *)value;\n"
      "return %s_validate_constraints(*typed_value, ec);\n"
      "}\n\n",
